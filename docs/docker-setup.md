...
```json
{
  "_comment": "generated by docker-setup.sh",
...
| Docker does not start after a settings change | A syntax error or an unknown key in `daemon.json`; on failure the script shows `journalctl -u docker` |
